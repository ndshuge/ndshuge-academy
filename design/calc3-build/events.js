...
scroller.addEventListener('click',function(e){
  ...
  if(opt){
    ...
    if(ok) noteOK('whyNote'+qi,'<b>答对了。</b>'+parseMath(q.explain||''));
    else noteNO('whyNote'+qi,'<b>答错了，正确答案：「'+parseMath(q.options[q.answer])+'」</b><br>'+parseMath(q.explain||'')+(q.type==='code'?'<br><button class="retry-btn" data-retry="'+qi+'">↻ 重新作答</button>':''));
  ...
  if(eopt){
    ...
    if(eok) noteOK('ewhyNote'+eqi,'<b>答对了。</b>'+parseMath(eq.explain||''));
    else noteNO('ewhyNote'+eqi,'<b>答错了，正确答案：「'+parseMath(eq.options[eq.answer])+'」</b><br>'+parseMath(eq.explain||'')+(eq.type==='code'?'<br><button class="retry-btn" data-etry="'+eqi+'">↻ 重新作答</button>':''));
  ...
  if(fgo){
    ...
    var fq=cF.quiz[fi];
    if(fok) noteOK('whyNote'+fi,'<b>答对了。</b>'+parseMath(fq.explain||''));
    else noteNO('whyNote'+fi,'<b>答错了。</b>'+parseMath(fq.explain||''));
  ...
  if(efgo){
    ...
    if(efok) noteOK('ewhyNote'+efi,'<b>答对了。</b>'+parseMath(efq.explain||''));
    else noteNO('ewhyNote'+efi,'<b>答错了。</b>'+parseMath(efq.explain||''));
...
function shareAcademy(){
  var url='https://ndshuge.github.io/ndshuge-academy/calculus.html?v='+encodeURIComponent(APP_VER);
  var done=function(){ toast('已复制学习链接'); };
  if(navigator.share){ navigator.share({title:'微积分学院',text:'跟我一起刷大一高数：极限 · 导数 · 积分：',url:url}).catch(function(){}); return; }
