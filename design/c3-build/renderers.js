...
renderers.home=function(){
  ...
  var nc=nextChapter();
  var h='<div class="kick">C LANGUAGE ACADEMY</div><div class="h1">C 语言学院</div>'
    +'<div class="sub">C 程序设计 50 章 · 语法到指针到文件 · 答完随堂题即点亮章节 · 进度 '+s.doneC+'/'+s.totalC+' 章</div>';
  h+='<div class="tutor-card"><span class="tutor-photo" style="display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#0A84FF,#5E5CE6);color:#fff;font-size:34px;font-family:Georgia,serif;letter-spacing:1px">DR</span>'
    +'<div class="tutor-body"><div class="tutor-cap">导读人</div>'
    +'<div class="tutor-name-big">DENNIS RITCHIE</div>'
    +'<div class="tutor-quote">'+pick(TUTOR_QUOTES)+'</div>'
    +'<div class="tutor-name">C 语言之父 · 贝尔实验室</div></div></div>';
  ...
  if(nc){
    ...
    h+='<div class="hero-card"><div class="deco">'+GEO+'</div>'
      ...
      +'<b class="next">50 章全部点亮。去荣誉墙领你的毕业称号。</b>'
  ...
  h+='<button class="big-entry lift" data-go="plan"><span class="be-ico" style="background:linear-gradient(135deg,#0A84FF,#5E5CE6)">'+IC('bookopen')+'</span>'
    ...
    +'<span class="cap">C 语言 50 章 · 讲义 + 随堂题，答完即点亮</span>'
...
var TUTOR_QUOTES=[
  '「C 是那样一门语言：它不迁就你，但也不骗你。」',
  '「指针不是武器，是地图——学会它，内存就不再神秘。」',
  '「分号是 C 的句号，漏了它编译器会提醒你。」',
  '「先读懂内存，再谈算法；先跑通，再谈优化。」',
  '「四十多年了，整个数字世界的底座还是 C。」',
  '「编译器的报错不是刁难，是它逐字读了你写的每一行。」']
/* ============ 学习 Tab · 章节课堂（50 章书架） ============ */
renderers.plan=function(){
  var s=calcStats();
  var h='<div class="kick">ROADMAP</div><div class="h1">章节课堂</div>'
    +'<div class="sub">五十章就是谭浩强《C 程序设计》。每章讲义 + 随堂题，答完即结算点亮。已学 '+s.doneC+'/'+s.totalC+'。</div>';
  CHAPTERS.forEach(function(c){
    var done=chIsDone(c);
    var sm=chSummary(c);
    var cap;
    if(done) cap='已学完'+(sm.allCorrect?' · 全对通关':' · 答完即结算');
    else cap=sm.answered>0?('进行中 '+sm.answered+'/'+sm.total+' 题 · 继续 →'):(c.full?'讲义已授 · 待开卷':'待里奇开讲');
...
function chapterHTML(c,mode){
  ...
  if(c.dialogue&&c.dialogue.q){
    ...
    h+='<div class="card dia"><h3>灵魂拷问（里奇的追问）</h3>'
...
function quizCardsHTML(c,mode){
  ...
  (c.quiz||[]).forEach(function(q,qi){
    ...
    h+='<div class="quiz'+(done?' done':'')+'" data-k="'+k+'">'
      +'<div class="q"><span class="qn">Q'+(qi+1)+'</span>'+(done?'<span class="hall-done-tag">✓ 已做</span>':'')+esc(q.q)+'</div>';
    if(q.type==='fill'){
      /* C 代码填空：template 内嵌 ___N___ 输入框 */
      var fdone=done;
      var ftpl=esc(q.template||'').replace(/___(\d+)___/g,'<input class="fill-in" data-n="$1" placeholder="填空 $1"'+(fdone?' disabled':'')+'>');
      var fshow=(done&&pv!==undefined)?pv:'';
      h+='<div class="codebox fillbox" data-fill="'+qi+'" data-key="'+k+'">'
        +'<div class="cb-head"><span class="lb">✍️ 代码填空</span>'+(q.expect?'<span class="lb" style="color:var(--muted)">期望输出：'+esc(String(q.expect).split('|').join(' 或 '))+'</span>':'')+'</div>'
        +'<pre class="fill-code">'+ftpl+'</pre>'
        +(q.blanks&&q.blanks.length?'<div class="fill-tips">'+q.blanks.map(function(b,bi){return '<span style="display:inline-block;background:var(--fill-soft);border-radius:8px;padding:3px 10px;margin:3px 6px 3px 0;font-size:12px;color:var(--ink2)">填空 '+(bi+1)+'：'+esc(b)+'</span>';}).join('')+'</div>':'')
        +'<div class="code-actions">'
        +'<button class="mini-btn lift" data-fillrun="'+qi+'"'+(fdone?' disabled':'')+'>▶ 运行判题</button>'
        +'<button class="mini-btn lift" data-fillans="'+qi+'" style="background:var(--fill-soft)"'+(fdone?' disabled':'')+'>👁 填参考</button>'
        +'</div>'
        +'<pre class="pyout" id="cout'+qi+'"'+(done?' style="display:block"':'')+'>'+(done&&pv!==undefined?esc(pv):'')+'</pre>'
        +'<div class="py-status" id="cfst'+qi+'"></div>'
        +(q.hint?'<div style="font-size:12.5px;color:var(--muted);margin-top:6px;line-height:1.6">💡 '+esc(q.hint)+'</div>':'')
        +'</div>';
    } else if(q.type==='code'){
      /* C 代码题：编辑器 + 运行/判题/看解（c99js 真编译） */
      var starter='';
      var showCode=(done&&pv!==undefined)?pv:(q.start||starter);
      h+='<div class="codebox" data-ccode="'+qi+'">'
        ...
        +'<textarea data-cm="c" class="py-ta" id="cta'+qi+'" placeholder="在下方编写代码…" spellcheck="false" data-cta="'+qi+'" data-starter="'+esc(starter)+'"'+(done?' readonly':'')+'>'+esc(showCode)+'</textarea>'
        +'<div class="code-actions">'
        +'<button class="mini-btn lift" data-crun="'+qi+'"'+(done?' disabled':'')+'>▶ 运行</button>'
        +'<button class="mini-btn lift" data-ccheck="'+qi+'" style="background:linear-gradient(135deg,#FF9F0A,#FF7A00);color:#fff"'+(done?' disabled':'')+'>✓ 判题</button>'
        +'<button class="mini-btn lift" data-cans="'+qi+'" style="background:var(--fill-soft)"'+(done?' disabled':'')+'>👁 参考解</button>'
        +'</div>'
        +'<pre class="pyout" id="cout'+qi+'"'+(done&&pv!==undefined?' style="display:block"':'')+'>'+(done&&pv!==undefined?esc(pv):'')+'</pre>'
        +'<div class="py-status" id="cfst'+qi+'"></div>'
        +(q.hint?'<div style="font-size:12.5px;color:var(--muted);margin-top:6px;line-height:1.6">💡 '+esc(q.hint)+'</div>':'')
        +'</div>';
    } else {
      ...
      ['A','B','C','D'].slice(0,(q.options||[]).length).forEach(function(k2,i){
        ...
        h+='<div class="'+cls+'" data-q="'+qi+'" data-i="'+i+'" data-o="'+i+'"'+(done?' data-lock="1"':'')+'>'+k2+'. '+esc(q.options[i])+'<span class="mark" style="display:none"></span></div>';
      });
      h+='</div>';
    }
    ...
    else if(hist===0) h+='<div class="why-body"><span class="why-ic"></span><span><b>上次答错，正确答案已标出：</b>'+esc(q.explain||'')+'</span></div>'+(q.type!=='choice'?'<button class="retry-btn" data-retry="'+qi+'">↻ 重新作答</button>':'');
    h+='</div></div>';
  });
  return h;
}
...
renderers.hall=function(){
  ...
  h+='<button class="tile lift" data-go="lab"><span class="t-ico" style="background:linear-gradient(135deg,#0A84FF,#5E5CE6)">'+IC('flask')+'</span><b>代码游乐场</b><span class="cap">随便写点 C</span></button>';
  h+='</div>';
...
function examHTML(c){
  ...
  (c.exam||[]).forEach(function(q,qi){
    ...
    var done=hist!==undefined;
    var pv=quizPick[k];
    var kind=(q.type==='fill')?'填空':(q.type==='code')?'编程':'单选';
    h+='<div class="card" style="border-radius:var(--r-l);margin-top:14px">'
      ...
      +'<span class="cap">'+kind+(hist!==undefined?(hist===1?' · 已答对':' · 上次答错'):'')+'</span></div>'
      +'<div class="q-stem">'+esc(q.q)+'</div><div id="eqbox'+qi+'" data-k="'+k+'">';
    if(q.type==='fill'){
      var ftpl=esc(q.template||'').replace(/___(\d+)___/g,'<input class="fill-in" data-n="$1" placeholder="填空 $1"'+(done?' disabled':'')+'>');
      h+='<div class="codebox fillbox" data-efill="'+qi+'" data-key="'+k+'">'
        +'<div class="cb-head"><span class="lb">✍️ 代码填空</span>'+(q.expect?'<span class="lb" style="color:var(--muted)">期望输出：'+esc(String(q.expect).split('|').join(' 或 '))+'</span>':'')+'</div>'
        +'<pre class="fill-code">'+ftpl+'</pre>'
        +(q.blanks&&q.blanks.length?'<div class="fill-tips">'+q.blanks.map(function(b,bi){return '<span style="display:inline-block;background:var(--fill-soft);border-radius:8px;padding:3px 10px;margin:3px 6px 3px 0;font-size:12px;color:var(--ink2)">填空 '+(bi+1)+'：'+esc(b)+'</span>';}).join('')+'</div>':'')
        +'<div class="code-actions">'
        +'<button class="mini-btn lift" data-efillrun="'+qi+'"'+(done?' disabled':'')+'>▶ 运行判题</button>'
        +'<button class="mini-btn lift" data-efillans="'+qi+'" style="background:var(--fill-soft)"'+(done?' disabled':'')+'>👁 填参考</button>'
        +'</div>'
        +'<pre class="pyout" id="ecout'+qi+'"'+(done?' style="display:block"':'')+'>'+(done&&pv!==undefined?esc(pv):'')+'</pre>'
        +'<div class="py-status" id="ecfst'+qi+'"></div>'
        +(q.hint?'<div style="font-size:12.5px;color:var(--muted);margin-top:6px;line-height:1.6">💡 '+esc(q.hint)+'</div>':'')
        +'</div>';
    } else if(q.type==='code'){
      var est='';
      var eshow=(done&&pv!==undefined)?pv:(q.start||est);
      h+='<div class="codebox" data-eccode="'+qi+'">'
        +'<div class="cb-head"><span class="lb">✏️ 写代码</span>'+(q.expect?'<span class="lb" style="color:var(--muted)">期望输出：'+esc(String(q.expect).split('|').join(' 或 '))+'</span>':'')+'</div>'
        +'<textarea class="py-ta" data-cm="c" id="ecta'+qi+'" placeholder="在下方编写代码…" spellcheck="false" data-ecta="'+qi+'" data-starter="'+esc(est)+'"'+(done?' readonly':'')+'>'+esc(eshow)+'</textarea>'
        +'<div class="code-actions">'
        +'<button class="mini-btn lift" data-ecrun="'+qi+'"'+(done?' disabled':'')+'>▶ 运行</button>'
        +'<button class="mini-btn lift" data-eccheck="'+qi+'" style="background:linear-gradient(135deg,#FF9F0A,#FF7A00);color:#fff"'+(done?' disabled':'')+'>✓ 判题</button>'
        +'<button class="mini-btn lift" data-ecans="'+qi+'" style="background:var(--fill-soft)"'+(done?' disabled':'')+'>👁 参考解</button>'
        +'</div>'
        +'<pre class="pyout" id="ecout'+qi+'"'+(done?' style="display:block"':'')+'>'+(done&&pv!==undefined?esc(pv):'')+'</pre>'
        +'<div class="py-status" id="ecfst'+qi+'"></div>'
        +(q.hint?'<div style="font-size:12.5px;color:var(--muted);margin-top:6px;line-height:1.6">💡 '+esc(q.hint)+'</div>':'')
      ...
      ['A','B','C','D'].slice(0,(q.options||[]).length).forEach(function(k2,i){
        var cls='opt'+(hist===0&&pv===i?' wrong':'')+(hist===1&&i===q.answer?' correct':(hist===0&&i===q.answer?' correct muted':''));
        h+='<button class="'+cls+'" data-eq="'+qi+'" data-i="'+i+'"'+(done?' data-lock="1"':'')+'><span class="k">'+k2+'</span><span>'+esc(q.options[i])+'</span><span class="mark">'+IC('check')+'</span></button>';
      });
    }
    ...
    else if(hist===0) h+='<div class="why-body"><span class="why-ic">'+IC('info')+'</span><span><b>上次答错。</b>'+esc(q.explain||'')+'</span></div>'
      +(q.type!=='choice'?'<button class="retry-btn" data-etry="'+qi+'">↻ 重新作答</button>':'');
    h+='</div></div>';
  });
  return h;
}
...
var PORTRAITS={}; /* C 学院无画像表（story.img 空），走首字母头像 */ /* 旧版荣誉墙名人像空表（无画像数据时走首字母头像分支） */
...
renderers.lab=function(){
  var h='<div class="kick">C SANDBOX</div><div class="h1">代码游乐场</div>'
    +'<div class="sub">不判题、不打卡，纯练手。随便写点 C，看它编译跑出什么。</div>';
  h+='<div class="card" style="border-radius:var(--r-l)">'
    +'<b>✏️ 试试这些：</b>'
    +'<div style="font-size:13px;color:var(--ink2);line-height:1.9;margin:6px 0 10px">'
    +'<code style="background:var(--fill-soft);padding:2px 6px;border-radius:6px">printf("Hello!\n")</code>　'
    ...
    +'<span style="color:var(--muted)">首次运行需加载 C 编译器（内置，约 2-5 秒）。</span></div>'
    +'<textarea class="py-ta" data-cm="c" id="sandTa" spellcheck="false" style="min-height:180px">#include <stdio.h>\nint main(void){\n    printf("Hello, C!\\n");\n    return 0;\n}</textarea>'
...
renderers.about=function(){
  return '<div class="kick">ABOUT</div><div class="h1">关于学院</div>'
    +'<div class="sub">C 语言学院 · framework v2 · 对齐样张 + HIG 动效</div>'
    +'<div class="card" style="border-radius:var(--r-l)"><div class="lesson-body">'
    +'<p>C 语言 50 章 · 语法、指针与文件操作循序渐进。讲义 + 随堂题（含代码填空与编程判题），答完点亮章节。</p>'
