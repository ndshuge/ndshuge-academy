...
OUT     = os.path.join(DESIGN, 'c3-真版-预览.html')
...
CODE_CSS = os.path.join(_HERE, '_code.css')
if os.path.exists(CODE_CSS):
    s = s[:si] + io.open(CODE_CSS, encoding='utf-8').read() + s[si:]
    print('code css injected')
...
# ---------- 5) 产物文案后处理（标题 / 静态品牌区） ----------
new = new.replace('<title>芯片学院 · 对齐样张 v2（iOS 26 玻璃语汇）</title>',
                  '<title>C 语言学院 · 谭浩强教程</title>')
new = new.replace('<b id="brandName">芯片战争学院</b><span>CHIP WAR ACADEMY</span>',
                  '<b id="brandName">C 语言学院</b><span>C LANGUAGE ACADEMY</span>')
