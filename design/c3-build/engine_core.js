/* ============================================================
   C 语言学院 · 真实引擎核心（键兼容线上：pyc_* 前缀 / daily_goal 全站共享）
   本段替换样张 demo 数据；渲染器与动效 100% 沿用样张。
   基线：chip2 定稿模板（引擎/云/守卫/settle 全套保留），仅换院参数。
   ============================================================ */
/* 学院参数（换院时改这里 + store 前缀 + RANKS/BADGES） */
var AC={id:'c',prefix:'cc_',app:'c'};
/* ---------- 工具 ---------- */
function pick(a){ return a[Math.floor(Math.random()*a.length)]; }
function esc(s){ return String(s==null?'':s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;'); }
function store(k,v){ try{ localStorage.setItem((k==='sb_session'||k==='profile'?'academy_':AC.prefix)+k, JSON.stringify(v)); }catch(e){} }
function load(k,d){ try{ var v=JSON.parse(localStorage.getItem((k==='sb_session'||k==='profile'?'academy_':AC.prefix)+k)); return v===null?d:v; }catch(e){ return d; } }
...
function dailyGoal(act){
  try{
    ...
    g[d]=g[d]||{}; g[d][AC.id]=g[d][AC.id]||{}; g[d][AC.id][act]=Date.now();
...
/* ---------- 称号 / 徽章 / 统计 ---------- */
var RANKS = [[0,'初入机房'],[4,'C 语新手'],[8,'编译魔法师'],[14,'输出小能手'],[20,'条件判断师'],[26,'循环跑者'],[32,'数组收藏家'],[38,'函数工匠'],[44,'指针猎手'],[48,'结构体建筑师'],[52,'文件管家'],[60,'C 语言行者'],[70,'指针大师'],[80,'内存掌控者'],[90,'C 语言传人'],[100,'C 语言宗师']];
...
var BADGES=[
  {n:'初识 C',d:'完成第 1 章',c:function(s){ return s.doneC>=1; }},
  {n:'Hello C',d:'跑通第一个 C 程序（第 2-4 章）',c:function(s){ return s.doneC>=4; }},
  {n:'调试入门',d:'完成第 5 章',c:function(s){ return s.doneC>=5; }},
  {n:'算法启蒙',d:'完成算法与流程图（6-9 章）',c:function(s){ return s.doneC>=9; }},
  {n:'printf 大师',d:'完成格式化输出（10 章）',c:function(s){ return s.doneC>=10; }},
  {n:'数据类型通',d:'完成数据类型与运算（11-16 章）',c:function(s){ return s.doneC>=16; }},
  {n:'选择结构',d:'完成 if/switch（17-20 章）',c:function(s){ return s.doneC>=20; }},
  {n:'循环行者',d:'完成循环（21-26 章）',c:function(s){ return s.doneC>=26; }},
  {n:'数组高手',d:'完成数组与字符串（27-32 章）',c:function(s){ return s.doneC>=32; }},
  {n:'函数工匠',d:'完成函数与递归（33-37 章）',c:function(s){ return s.doneC>=37; }},
  {n:'指针猎手',d:'完成指针（38-42 章）',c:function(s){ return s.doneC>=42; }},
  {n:'结构体',d:'完成结构体与枚举（43-46 章）',c:function(s){ return s.doneC>=46; }},
  {n:'文件操作',d:'完成文件读写（47-50 章）',c:function(s){ return s.doneC>=50; }},
  {n:'错题清零',d:'错题全部订正',c:function(s){ return s.wrongPool===0&&s.fixedPool>0; }},
  {n:'学习启航',d:'累计学习 5 次',c:function(s){ return s.studyCount>=5; }},
  {n:'今日打卡',d:'今日完成一章练习或一份卷（四院共通）',c:function(s){ try{ var g=JSON.parse(localStorage.getItem('daily_goal')||'{}'); var d0=g[todayStr()]; return !!(d0&&d0[AC.id]&&(d0[AC.id].l||d0[AC.id].e)); }catch(e){ return false; } }},
...
/* ---------- 清除（游客初始态） ---------- */
function wipeAll(){
  ['ch','hall','quizStat','quizPick','log','settle','tstat','stat','wrong','fixed','checkin','navfold','sidefold'].forEach(function(k){ try{ localStorage.removeItem(AC.prefix+k); }catch(e){} });
  clearDailyGoal(AC.id);
}
