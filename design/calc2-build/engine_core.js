/* ============================================================
   学院引擎核心（参数化：AC = {id, prefix, app} 由构建注入）
   ============================================================ */
...
var BADGES=[
  ...
  {n:'行百里者', d:'完成全书 25%', c:function(s){ return s.doneC>=Math.ceil(s.totalC*0.25); }},
  {n:'半程点灯', d:'完成全书一半', c:function(s){ return s.doneC>=Math.ceil(s.totalC*0.5); }},
  {n:'卷上轻舟', d:'完成第一份小卷', c:function(s){ return s.examFull>=1; }},
  {n:'错题清零', d:'错题全部订正', c:function(s){ return s.wrongPool===0 && s.fixedPool>0; }},
  {n:'今日打卡', d:'今日完成学习（四院共通）', c:function(s){ try{ var g=JSON.parse(localStorage.getItem('daily_goal')||'{}'); var d0=g[todayStr()]; return !!(d0&&d0[AC.id]&&(d0[AC.id].l||d0[AC.id].e)); }catch(e){ return false; } }},
  {n:'毕业学士', d:'全书通关', c:function(s){ return s.doneC>=s.totalC; }}
];
