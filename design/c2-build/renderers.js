...
renderers.home=function(){
  ...
  var nc=nextChapter();
  var h='<div class="kick">C LANGUAGE</div><div class="h1">C 语言学院</div>'
    +'<div class="sub">指针与内存 · 浏览器真编译 · 进度 '+s.doneC+'/'+s.totalC+' 章</div>';
  ...
  h+='<div class="tutor-card"><img class="tutor-photo" alt="导读人" src="TUTOR_IMG_PLACEHOLDER">'
    +'<div class="tutor-body"><div class="tutor-cap">导读人 · 克尼汉</div>'
    +'<div class="tutor-quote">'+pick(TUTOR_QUOTES)+'</div>'
    +'<div class="tutor-name">「C 是语言的母亲」 · 教科书级作者</div></div></div>';
  return h;
};
var TUTOR_QUOTES=[ '「指针是 C 的灵魂：它让你亲手握住内存。」', '「先学会把程序拆成函数，再谈别的。」', '「每一块内存都有主人，用完要还。」', '「数组名不是指针，但总会让人误会。」', '「编译过了只是开始，跑对了才是结束。」', '「用 C 写一遍，才真正懂计算机。」' ];
...
function chapterHTML(c,mode){
  ...
  (c.sections||[]).forEach(function(sec){
    ...
    if(sec.formula) h+='<div style="background:var(--fill-soft);border-radius:10px;padding:9px 13px;font-size:15px;color:var(--ink2);font-weight:600">'+esc(sec.formula)+'</div>'; else if(sec.code) h+='<pre class="q-code">'+esc(sec.code)+'</pre>';
  });
  if(c.story){
...
renderers.mine=function(){
  ...
  h+='<div class="card" style="display:flex;align-items:center;gap:14px;border-radius:var(--r-l)">'
    ...
    +'<div style="min-width:0;flex:1"><b style="font-size:18px">C 学徒</b><div style="font-size:13px;color:var(--ink2);margin-top:2px;line-height:1.5;min-width:0">'+rk.name+' · 已学 '+s.doneC+'/'+s.totalC+' 章 · 小卷 '+s.examFull+' 份</div>'
...
renderers.lab=function(){
  var h='<div class="kick">C LAB</div><div class="h1">实验室</div><div class="sub">指针是 C 的灵魂：它让你亲手握住内存</div>';
  h+='<div class="card" style="border-radius:var(--r-l)"><b>指针示意 · 内存格子</b>'
    +'<canvas id="cvMem" width="720" height="240" style="width:100%;height:auto;background:#fff;border-radius:12px;margin-top:10px"></canvas>'
    +'<div style="display:flex;align-items:center;gap:12px;margin-top:12px;flex-wrap:wrap">'
    +'<input type="range" id="memS" min="0" max="7" step="1" value="2" style="flex:1;min-width:140px">'
    +'<button class="mini-btn lift" id="memGo" style="flex:none">走一格</button></div>'
    +'<div class="status" id="memSt" style="font-size:13px;color:var(--ink2);margin-top:10px;line-height:1.6"></div></div>';
  setTimeout(function(){ try{ cMemDraw(); }catch(e){} },50);
  return h;
};
function cMemDraw(){
  var cv=document.getElementById('cvMem'); if(!cv||!cv.getContext) return;
  var c=cv.getContext('2d'); var i=parseInt(document.getElementById('memS').value||0,10);
  var W=cv.width,H=cv.height, n=8, cell=Math.min(72,(W-80)/n);
  c.fillStyle='#fff'; c.fillRect(0,0,W,H);
  var names=['a','b','c','d','e','f','g','h'],vals=[42,7,99,13,5,77,20,3];
  for(var k=0;k<n;k++){
    var x=60+k*(cell+14), y=50;
    c.fillStyle='#F2F3F7'; c.strokeStyle='#C7C9D1';
    c.beginPath(); c.roundRect?c.roundRect(x,y,cell,86,10):c.rect(x,y,cell,86); c.fill(); c.stroke();
    c.fillStyle='#3C3C43'; c.font='13px ui-monospace,Consolas,monospace';
    c.fillText('0x'+('10'+k).toString(16), x, y+16);
    c.fillStyle='#007AFF'; c.font='bold 22px ui-monospace,Consolas,monospace';
    c.fillText(''+vals[k], x+cell/2-12, y+56);
    c.fillStyle='#86868B'; c.font='12px sans-serif'; c.fillText(names[k], x+cell/2-6, y+78);
  }
  var ax=60+i*(cell+14);
  c.strokeStyle='#FF3B30'; c.lineWidth=3; c.beginPath(); c.moveTo(ax+cell/2, y-20); c.lineTo(ax+cell/2, y+6); c.stroke();
  c.fillStyle='#FF3B30'; c.font='bold 13px ui-monospace,Consolas,monospace';
  c.fillText('int *p = &'+names[i]+';', 60, H-22);
  c.fillStyle='#3C3C43'; c.font='13px sans-serif'; c.fillText('*p = '+vals[i], 300, H-22);
  var st=document.getElementById('memSt');
  if(st) st.textContent='指针 p 指向第 '+i+' 格（'+names[i]+'），*p 读出 '+vals[i]+'。p+1 会走到下一个格子。';
}
...
renderers.about=function(){
  return '<div class="kick">ABOUT</div><div class="h1">关于学院</div>'
    +'<div class="sub">C 语言学院 · 样张真版 '+APP_VER+'</div>'
    +'<div class="card" style="border-radius:var(--r-l)"><div class="lesson-body"><p>以「对齐样张 v2」为界面底座（iOS 26 玻璃语汇），接入 <b>C 语言学院真实内容引擎</b>：36 章讲义、随堂判题、章节结算、小卷、错题池、称号徽章与跨院打卡，全部读写线上同款存储键。</p>'
...
renderers.author=function(){
  return '<div class="kick">BACKSTAGE</div><div class="h1">作者与幕后</div>'
    ...
    +'接着 C 语言学院（浏览器里真编译真运行）、C 语言学院（读透《芯片战争》）陆续加入。<br>'
