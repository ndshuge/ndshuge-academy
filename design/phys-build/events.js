...
function shareAcademy(){
  var url='https://ndshuge.github.io/ndshuge-academy/phys.html?v='+encodeURIComponent(APP_VER);
  var done=function(){ toast('已复制学习链接'); };
  if(navigator.share){ navigator.share({title:'大学物理学院',text:'跟我一起学《大学物理学》：',url:url}).catch(function(){}); return; }
...
function showStart(kind,id){
  ...
  if(kind==='exam'){
    ...
    title=esc(c.title);
    sub=chIsDone(c)?'本章已点亮 · 可回来重读重做':(c.full?(isD?'按章刷题 · 答对自动移出错题':'讲义 + 随堂题 · 答完自动结算点亮章节'):'待吴百诗开讲');
