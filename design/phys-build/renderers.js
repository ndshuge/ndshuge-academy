...
renderers.home=function(){
  ...
  var nc=nextChapter();
  var h='<div class="kick">PHYSICS ACADEMY</div><div class="h1">大学物理学院</div>'
    +'<div class="sub">吴百诗《大学物理学》51 章 · 答完随堂题即点亮章节 · 进度 '+s.doneC+'/'+s.totalC+' 章</div>';
  h+='<div class="tutor-card"><img class="tutor-photo" alt="导读人吴百诗" src="TUTOR_IMG_PLACEHOLDER">'
    +'<div class="tutor-body"><div class="tutor-cap">导读人</div>'
    +'<div class="tutor-name-big">WU BAISHI</div>'
    +'<div class="tutor-quote">'+pick(TUTOR_QUOTES)+'</div>'
    +'<div class="tutor-name">《大学物理学》作者 · 西安交通大学</div></div></div>';
  ...
  if(nc){
    ...
    h+='<div class="hero-card"><div class="deco">'+GEO+'</div>'
      ...
      +'<b class="next">51 章全部点亮。去荣誉墙领你的毕业称号。</b>'
  ...
  h+='<button class="big-entry lift" data-go="plan"><span class="be-ico" style="background:linear-gradient(135deg,#0A84FF,#5E5CE6)">'+IC('bookopen')+'</span>'
    ...
    +'<span class="cap">吴百诗读本 51 章 · 讲义 + 随堂题，答完即点亮</span>'
...
var TUTOR_QUOTES=[
  '「物理学的大厦，建立在几条最朴素的事实之上。」',
  '「先想清楚受力与能量去了哪里，再动笔列方程。」',
  '「一个公式背后的图像，比公式本身重要得多。」',
  '「从落体到电磁，自然的规律总比我们想的更统一。」',
  '「量纲先看一眼，数量级再估一遍，答案自己会告诉你对不对。」',
  '「你读的不是公式全集，是这世界如何运转的说明书。」'
];
...
renderers.plan=function(){
  ...
  var h='<div class="kick">ROADMAP</div><div class="h1">章节课堂</div>'
    +'<div class="sub">六篇五十一章，从质点一路走到量子。每章讲义 + 随堂 10 题，答完即结算点亮。已学 '+s.doneC+'/'+s.totalC+'。</div>';
  CHAPTERS.forEach(function(c){
    var done=chIsDone(c);
    var sm=chSummary(c);
    var cap;
    if(done) cap='已学完'+(sm.allCorrect?' · 全对通关':' · 答完即结算');
    else cap=sm.answered>0?('进行中 '+sm.answered+'/'+sm.total+' 题 · 继续 →'):(c.full?'讲义已授 · 待开卷':'待吴百诗开讲');
...
function chapterHTML(c,mode){
  ...
  if(c.story){
    ...
    h+='<div class="card story-card"><h3>物理史话 · 这一章的来处</h3>'
  ...
  /* 例题（引号内独白式推导） */
  if(c.example){
    h+='<div class="card"><h3>例题 · 推导的内心独白</h3><div class="example"><div class="lbl">SOLVE WITH YOUR INNER MONOLOGUE</div>'
      +'<p><b>'+parseMath(c.example.q)+'</b></p><p>'+parseMath(c.example.s)+'</p>'
      +'<p style="font-size:13px;color:var(--ink2);margin-top:8px">看求解过程教不会你思考。先挡住解答自己试做，再对照这段「内心独白」——注意看思路是怎么来的，而不是只看写了什么。</p>'
      +'</div></div>';
  }
  /* 易错陷阱 */
  ...
  if(c.dialogue&&c.dialogue.q){
    ...
    h+='<div class="card dia"><h3>灵魂拷问（学完自问）</h3>'
...
renderers.calendar=function(){
  ...
  var h='<div class="kick">CALENDAR</div><div class="h1">打卡日历</div>'
    +'<div class="sub">全站多院共通，跨学院不断签。今日连签 '+streak+' 天</div>';
  var now=new Date();
...
/* ============ 荣誉墙（整块移用自物理学院样张：称号阶梯 + 徽章墙 + 名人堂 + 资料卡） ============ */
...
renderers.honor=function(){
  ...
  var h='<div class="kick">HONOR</div><div class="h1">荣誉墙</div>'
    +'<div class="sub">称号按已学章节推进，徽章记录你的历史时刻，学完一章点亮一位物理学家</div>'
    +'<div id="legacy-honor">'
    ...
    +'<div class="card"><h3>📜 物理学家名人堂</h3><p class="hint">学完本章点亮对应物理学家，点亮后可点击查看资料卡。学完的越多，名人堂越亮。</p><div class="hall-grid">'+hall+'</div></div>'
    +'</div>';
  setTimeout(function(){
...
renderers.lab=function(){
  return '<div class="kick">THE PHYSICS LAB</div><div class="h1">物理实验室</div>'
    +'<div class="sub">抛体 · 单摆 · 库仑力 · 波的叠加（下一批接入画布）</div>'
    +'<div class="card" style="text-align:center;padding:30px 20px"><span style="font-size:44px">🔬</span>'
    +'<div class="cap" style="margin-top:8px;line-height:1.7">四个真实物理模拟器（平抛轨迹 / 单摆测 g / 库仑定律 / 波的干涉）正在搬入<br>先把书读起来，实验台随后就到。</div></div>';
};
...
renderers.about=function(){
  return '<div class="kick">ABOUT</div><div class="h1">关于学院</div>'
    +'<div class="sub">大学物理学院 · framework v2 · 对齐样张 + HIG 动效</div>'
    +'<div class="card" style="border-radius:var(--r-l)"><div class="lesson-body">'
    +'<p>吴百诗《大学物理学》51 章 · 从质点运动一路走到量子与相对论。讲义 + 随堂题，答完点亮章节。</p>'
    +'<p>界面与动效统一走 <b>Apple 设计语言（framework v2）</b>：双舞台转场、Interactive Pop 跟手返回、Sheet 环境光、滚动视觉降噪、Dock 款悬停；内容为本院真实全书数据，进度按院存于本机（键隔离），打卡全站多院共通。</p>'
    +'<p>导读人：吴百诗。称号与徽章在荣誉墙可见。</p>'
...
renderers.author=function(){
  return '<div class="kick">BACKSTAGE</div><div class="h1">作者与幕后</div>'
    ...
    +'接着 C 语言学院（浏览器里真编译真运行）、芯片战争学院（读透《芯片战争》）。<br>'
    +'最新加入的是大学物理学院（吴百诗《大学物理学》51 章，例题、随堂题与小卷都是真物理）。<br>'
    +'五院并列，成了现在的鼠哥学院：一个链接，几门硬课，一群同学。</div>'
...
/* ============ 物理实验室（真画布：平抛/单摆/库仑/干涉） ============ */
renderers.lab=function(){
  function ctl(sl, v, st, unit){
    return '<div class="controls" style="display:flex;align-items:center;gap:12px;margin:10px 0;flex-wrap:wrap">'
      +'<label style="font-size:13px;color:var(--ink2)">'+unit+' = <b class="val" id="'+v+'">0</b></label>'
      +'<input type="range" id="'+sl+'" style="flex:1;min-width:140px" min="0" max="100" step="1" value="0"></div>';
  }
  var h='<div class="kick">THE PHYSICS LAB</div><div class="h1">物理实验室</div>'
    +'<div class="sub">不是看，是动手：平抛、单摆、库仑力、波的叠加，每个滑块背后都是书里真用过的公式。</div>';
  h+='<div class="card" style="border-radius:var(--r-l)"><b>实验一 · 平抛：初速与射程</b>'
    +'<p style="font-size:13.5px;color:var(--ink2);margin:6px 0;line-height:1.7">拖动<b>水平初速度</b>：竖直方向永远是自由落体，落地时间只由高度决定，与水平初速无关。</p>'
    +ctl('vS','vV','水平初速 v₀','v₀')+'<canvas id="cvProj" width="720" height="230" style="width:100%;height:auto;border-radius:10px"></canvas>'
    +'<div class="status" id="stProj" style="font-size:13px;color:var(--ink2);margin-top:8px;line-height:1.6"></div></div>';
  h+='<div class="card" style="border-radius:var(--r-l);margin-top:12px"><b>实验二 · 单摆：摆长决定周期</b>'
    +'<p style="font-size:13.5px;color:var(--ink2);margin:6px 0;line-height:1.7">拖动<b>摆长</b>：小角度下单摆周期 T = 2π√(L/g)，与振幅、与摆球质量都无关。</p>'
    +ctl('pendS','pendV','摆长 L','L')+'<canvas id="cvPend" width="720" height="240" style="width:100%;height:auto;border-radius:10px"></canvas>'
    +'<div class="status" id="stPend" style="font-size:13px;color:var(--ink2);margin-top:8px;line-height:1.6"></div></div>';
  h+='<div class="card" style="border-radius:var(--r-l);margin-top:12px"><b>实验三 · 库仑力：平方反比定律</b>'
    +'<p style="font-size:13.5px;color:var(--ink2);margin:6px 0;line-height:1.7">拖动<b>两电荷间距</b>：受力按 1/r² 变化，距离翻倍，斥力只剩四分之一。</p>'
    +ctl('coulS','coulV','间距 r','r')+'<canvas id="cvCoul" width="720" height="230" style="width:100%;height:auto;border-radius:10px"></canvas>'
    +'<div class="status" id="stCoul" style="font-size:13px;color:var(--ink2);margin-top:8px;line-height:1.6"></div></div>';
  h+='<div class="card" style="border-radius:var(--r-l);margin-top:12px"><b>实验四 · 波的叠加：干涉相长与相消</b>'
    +'<p style="font-size:13.5px;color:var(--ink2);margin:6px 0;line-height:1.7">拖动<b>相位差</b>：两列同频波叠加，相位差决定合振幅是加倍还是抵消。</p>'
    +ctl('waveS','waveV','相位差 Δφ','Δφ')+'<canvas id="cvWave" width="720" height="250" style="width:100%;height:auto;border-radius:10px"></canvas>'
    +'<div class="status" id="stWave" style="font-size:13px;color:var(--ink2);margin-top:8px;line-height:1.6"></div></div>';
  setTimeout(function(){
    try{ drawProj(); drawPend(); drawCoul(); drawWave(); }catch(e){}
  },30);
  return h;
};
...
/* 实验室画布（物理版） */
function drawProj(){
  var cv=document.getElementById('cvProj'); if(!cv || !cv.getContext) return;
  var c=cv.getContext('2d'); var v=+document.getElementById('vS').value;
  c.clearRect(0,0,cv.width,cv.height);
  var x0=60, y0=40, H=cv.height-80;
  c.strokeStyle='#2b3440'; c.beginPath(); c.moveTo(x0,y0); c.lineTo(x0,y0+H); c.lineTo(x0+cv.width-100,y0+H); c.stroke();
  c.fillStyle='#7d8896'; c.font='12px sans-serif';
  c.fillText('h = 20 m', x0-20, y0+H/2); c.fillText('地面', x0+cv.width-105, y0+H+16);
  var g=9.8, h0=20;
  var t=Math.sqrt(2*h0/g);
  var vms=2+v*0.18;
  var range=vms*t;
  var scaleM=(cv.width-160)/Math.max(range,1);
  var px0=x0, py0=y0;
  var px1=x0+range*scaleM, py1=y0+H;
  c.strokeStyle='#3fb950'; c.lineWidth=2; c.beginPath();
  var N=40;
  for(var i=0;i<=N;i++){
    var tt=t*i/N, xx=px0+vms*tt*scaleM, yy=py0+0.5*g*tt*tt*(H/h0);
    if(i===0) c.moveTo(xx,yy); else c.lineTo(xx,yy);
  }
  c.stroke(); c.lineWidth=1;
  c.fillStyle='#e3b341'; c.beginPath(); c.arc(px1,py1,6,0,7); c.fill();
  c.fillStyle='#d8e1ea'; c.font='14px sans-serif';
  c.fillText('v₀ = '+vms.toFixed(1)+' m/s', x0, y0-14);
  c.fillText('水平射程 x ≈ '+range.toFixed(1)+' m', Math.max(x0,px1-70), py1-16);
  var st=document.getElementById('stProj');
  st.textContent = '落地时间 t = √(2h/g) ≈ '+t.toFixed(2)+' s，只由高度 h = '+h0+' m 决定；初速越大只跑得越远，不会掉得越慢。射程 x = v₀t = '+range.toFixed(1)+' m。';
}

function drawPend(){
  var cv=document.getElementById('cvPend'); if(!cv || !cv.getContext) return;
  var c=cv.getContext('2d'); var s=+document.getElementById('pendS').value;
  c.clearRect(0,0,cv.width,cv.height);
  var ox=cv.width*0.30, oy=30, L=60+s*1.4, g=9.8;
  var T=2*Math.PI*Math.sqrt((L/100)/g);
  var ang=18*Math.PI/180;
  var bx=ox+L*Math.sin(ang), by=oy+L*Math.cos(ang);
  c.strokeStyle='#7d8896'; c.lineWidth=2; c.beginPath(); c.moveTo(ox,oy); c.lineTo(bx,by); c.stroke(); c.lineWidth=1;
  c.fillStyle='#3fb950'; c.beginPath(); c.arc(ox,oy,4,0,7); c.fill();
  c.fillStyle='#e3b341'; c.beginPath(); c.arc(bx,by,13,0,7); c.fill();
  c.setLineDash([5,5]); c.strokeStyle='#2b3440'; c.beginPath(); c.moveTo(ox,oy); c.lineTo(ox,oy+L+30); c.stroke(); c.setLineDash([]);
  c.fillStyle='#7d8896'; c.font='12px sans-serif'; c.fillText('竖直基准', ox+6, oy+L+42);
  var bx0=cv.width*0.56, by0=cv.height*0.72, bw=cv.width*0.34, bh=16;
  c.fillStyle='#1a212b'; c.fillRect(bx0,by0,bw,bh);
  var frac=Math.min(1,Math.max(0,(T-0.5)/2.5));
  c.fillStyle='#0A84FF'; c.fillRect(bx0,by0,bw*frac,bh);
  c.strokeStyle='#2b3440'; c.strokeRect(bx0,by0,bw,bh);
  c.fillStyle='#d8e1ea'; c.font='14px sans-serif';
  c.fillText('摆长 L = '+(L/100).toFixed(2)+' m', bx0, by0-46);
  c.fillText('周期 T = 2π√(L/g) ≈ '+T.toFixed(2)+' s', bx0, by0-24);
  c.fillStyle='#7d8896'; c.font='12px sans-serif';
  c.fillText('T 与振幅、摆球质量无关', bx0, by0+bh+22);
  var st=document.getElementById('stPend');
  st.textContent = '摆长越长周期越长，且是开方关系：摆长变 4 倍，周期只变 2 倍。这是单摆测重力加速度的实验基础。';
}

function drawCoul(){
  var cv=document.getElementById('cvCoul'); if(!cv || !cv.getContext) return;
  var c=cv.getContext('2d'); var s=+document.getElementById('coulS').value;
  c.clearRect(0,0,cv.width,cv.height);
  var cy=cv.height/2;
  var r=80+s*2.0;
  var cx=cv.width/2;
  var q1=6, q2=6, k=8.99e9;
  var F=k*q1*q2/Math.pow(r/100,2);
  var x1=cx, x2=cx+r;
  c.fillStyle='#FF453A'; c.beginPath(); c.arc(x1,cy,16,0,7); c.fill();
  c.fillStyle='#0A84FF'; c.beginPath(); c.arc(x2,cy,16,0,7); c.fill();
  c.fillStyle='#fff'; c.font='bold 15px sans-serif'; c.textAlign='center';
  c.fillText('+', x1, cy+5); c.fillText('+', x2, cy+5); c.textAlign='start';
  var arrowLen=Math.min(90, 20000/(r*r*0.005+1)+20);
  c.strokeStyle='#e3b341'; c.lineWidth=3;
  c.beginPath(); c.moveTo(x1+20,cy); c.lineTo(x1+20-arrowLen,cy); c.stroke();
  c.beginPath(); c.moveTo(x1+20-arrowLen,cy); c.lineTo(x1+32-arrowLen,cy-7); c.lineTo(x1+32-arrowLen,cy+7); c.closePath(); c.fillStyle='#e3b341'; c.fill();
  c.beginPath(); c.moveTo(x2-20,cy); c.lineTo(x2-20+arrowLen,cy); c.stroke();
  c.beginPath(); c.moveTo(x2-20+arrowLen,cy); c.lineTo(x2-32+arrowLen,cy-7); c.lineTo(x2-32+arrowLen,cy+7); c.closePath(); c.fill();
  c.lineWidth=1;
  c.setLineDash([4,4]); c.strokeStyle='#7d8896'; c.beginPath(); c.moveTo(x1,cy-40); c.lineTo(x2,cy-40); c.stroke(); c.setLineDash([]);
  c.fillStyle='#d8e1ea'; c.font='14px sans-serif';
  c.fillText('r = '+(r/100).toFixed(2)+' m', (x1+x2)/2-30, cy-48);
  c.fillText('F ≈ '+F.toExponential(2)+' N', 40, 34);
  c.fillStyle='#7d8896'; c.font='12px sans-serif';
  c.fillText('两个同号电荷，斥力沿连线方向', 40, cv.height-14);
  var st=document.getElementById('stCoul');
  st.textContent = '库仑定律 F = kq₁q₂/r²：距离翻倍，力降到四分之一；距离变 3 倍，力只剩九分之一。平方反比是静电场一切结论的源头。';
}

function drawWave(){
  var cv=document.getElementById('cvWave'); if(!cv || !cv.getContext) return;
  var c=cv.getContext('2d'); var s=+document.getElementById('waveS').value;
  c.clearRect(0,0,cv.width,cv.height);
  var dphi=s/100*2*Math.PI;
  var yA=cv.height*0.24, yB=cv.height*0.50, yS=cv.height*0.78;
  var x0=40, W=cv.width-80, A=24, k=2*Math.PI/W*3;
  function curve(yBase, fn, color){
    c.strokeStyle=color; c.lineWidth=2; c.beginPath();
    for(var x=0;x<=W;x++){
      var yy=yBase+fn(x);
      if(x===0) c.moveTo(x0+x,yy); else c.lineTo(x0+x,yy);
    }
    c.stroke(); c.lineWidth=1;
  }
  curve(yA, function(x){ return A*Math.sin(k*x); }, '#0A84FF');
  curve(yB, function(x){ return A*Math.sin(k*x+dphi); }, '#34C759');
  var amp=2*A*Math.abs(Math.cos(dphi/2));
  curve(yS, function(x){ return A*Math.sin(k*x)+A*Math.sin(k*x+dphi); }, '#e3b341');
  c.fillStyle='#7d8896'; c.font='12px sans-serif';
  c.fillText('波 1', x0, yA-A-6); c.fillText('波 2', x0, yB-A-6);
  c.fillText('合振动（振幅 '+amp.toFixed(0)+' / 最大 '+(2*A)+'）', x0, yS-A-8);
  var st=document.getElementById('stWave');
  if(amp>1.8*A) st.textContent='相位差接近 0：两列波步调一致，合振幅几乎是单列的两倍，干涉相长。';
  else if(amp<0.25*A) st.textContent='相位差接近 180°：两列波完全反着走，互相抵消，合振幅趋近于零，干涉相消。';
  else st.textContent='相位差 '+Math.round(dphi*180/Math.PI)+'°：合振幅 A合 = 2A·|cos(Δφ/2)| ≈ '+amp.toFixed(0)+'，介于相长与相消之间。';
}

/* ===== 物理学家名人堂（学完本章点亮，点亮可点开资料卡） ===== */
function physicsHallHTML(){
  var palette=[['#0A84FF','#5E5CE6'],['#FF9F0A','#FF7A00'],['#FF453A','#D70015'],['#34C759','#28A745'],['#AF52DE','#7D2AE0'],['#5856D6','#AF52DE']];
  var cards = CHAPTERS.map(function(c){
    if(!c.story || !c.story.tutor) return '';
    var nm = c.story.tutor.replace(/（.*/,'').trim();
    var lit = chIsDone(c);
    var p2 = palette[(c.id-1) % palette.length];
    var quote = (c.story.quote || '').replace(/^「/,'').replace(/」.*$/,'').trim();
    return '<button class="hall-card'+(lit?' lit':'')+'" data-tutor="'+c.id+'" style="text-align:left;border:1px solid '+(lit?'rgba(0,122,255,.4)':'var(--sep)')+';background:var(--card);border-radius:18px;padding:13px 14px;display:flex;gap:12px;align-items:center;width:100%;box-shadow:var(--sh-card);cursor:pointer">'
      +'<span style="width:46px;height:46px;border-radius:50%;background:linear-gradient(135deg,'+p2[0]+','+p2[1]+');display:flex;align-items:center;justify-content:center;color:#fff;font-size:18px;flex:none;font-family:Georgia,serif">'+esc(nm.charAt(0))+'</span>'
      +'<span style="flex:1;min-width:0"><b style="font-size:14.5px;color:var(--ink)">'+esc(nm)+'</b>'
      +'<span class="cap" style="display:block;font-size:11.5px;margin-top:2px;line-height:1.5">'+esc(c.story.era||'')+' · 第 '+c.id+' 章</span>'
      +(quote?'<span class="cap" style="display:block;font-size:11.5px;margin-top:2px;color:var(--muted);line-height:1.5">「'+esc(quote)+'」</span>':'')+'</span>'
      +(lit?'<span>'+IC('chev-r')+'</span>':'<span class="cap" style="font-size:11px">🔒 学完点亮</span>')
      +'</button>';
  }).join('');
  return '<div class="card" style="border-radius:var(--r-l);margin-top:14px"><b style="font-size:16px">📜 物理学家名人堂</b>'
    +'<div class="cap" style="margin:4px 0 12px;line-height:1.7">学完本章点亮对应物理学家，点亮后可点开资料卡。学得越多，名人堂越亮。</div>'
    +'<div style="display:grid;grid-template-columns:1fr;gap:10px">'+cards+'</div></div>';
}
function physicsProfile(c){
  if(!c || !c.story) return;
  var nm = c.story.tutor.replace(/（.*/,'').trim();
  var palette=[['#0A84FF','#5E5CE6'],['#FF9F0A','#FF7A00'],['#FF453A','#D70015'],['#34C759','#28A745'],['#AF52DE','#7D2AE0'],['#5856D6','#AF52DE']];
  var p2 = palette[(c.id-1) % palette.length];
  ...
  var box=document.createElement('div');
  box.style.cssText='background:var(--card);border-radius:26px;padding:26px;max-width:560px;width:100%;max-height:86vh;overflow:auto;box-shadow:var(--sh-float);transform:translateY(24px) scale(.97);opacity:0;transition:transform .4s cubic-bezier(.24,1.42,.4,1),opacity .3s';
  requestAnimationFrame(function(){ box.style.transform='translateY(0) scale(1)'; box.style.opacity='1'; });
  box.innerHTML='<div style="display:flex;gap:16px;align-items:center">'
    +'<span style="width:74px;height:74px;border-radius:22px;background:linear-gradient(135deg,'+p2[0]+','+p2[1]+');display:flex;align-items:center;justify-content:center;color:#fff;font-size:30px;flex:none;font-family:Georgia,serif">'+esc(nm.charAt(0))+'</span>'
    +'<div style="min-width:0"><b style="font-size:19px">'+esc(nm)+'</b><div class="cap" style="margin-top:3px">'+esc(c.story.era||'')+' · 第 '+c.id+' 章人物</div></div></div>'
    +'<div style="font-size:14px;color:var(--ink2);line-height:1.9;margin-top:16px">'+parseMath(c.story.fact||'')+'</div>'
    +(c.story.quote?'<div style="margin-top:14px;padding:12px 15px;border-left:3px solid var(--accent);background:var(--fill-soft);border-radius:10px;color:var(--ink2);font-size:14px;line-height:1.7">'+parseMath(c.story.quote)+'</div>':'')
    +(c.story.story?'<div style="margin-top:12px;font-size:13.5px;color:var(--muted);line-height:1.8">轶事 · '+parseMath(c.story.story)+'</div>':'');
  var btn=document.createElement('button');
  btn.textContent='知道了';
  btn.style.cssText='margin-top:18px;width:100%;height:46px;border:none;border-radius:14px;background:var(--accent);color:#fff;font:600 15px inherit;font-family:inherit;cursor:pointer';
